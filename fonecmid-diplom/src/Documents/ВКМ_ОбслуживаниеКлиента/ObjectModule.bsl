...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	И Дата <= Выборка.ДатаОкончания И Дата >= Выборка.ДатаНачала) Тогда
		...
		Если Дата >= Выборка.ДатаОкончания Тогда
			ТекстСообщения = "У договора %1 закончился срок действия";
		Иначе
			ТекстСообщения = "У договора %1 срок действия не начался"
		КонецЕсли;
		Сообщение.Текст = СтрШаблон(ТекстСообщения, Выборка.Наименование);
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
